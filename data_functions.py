...
def getFeatures(path, name, player_name):
    ...
    try:
        ...
        rhythm_feature, velocity_feature, duration_feature, pitch_feature, tempo_feature = Performance_class.Performance(
            path=performance_path, name=name, player_name=player_name, original_path=original_path).get_features()
...
def getDataForSL(csv_path, number_of_teachers, folder_path):
    ...
    return all_performances_predications, features_predications


def trainAndTest(csv_path, number_of_teachers, folder_path, train_ratio, random_teachers=True, features=False,
                 fake_data=0):
    all_performances_predications, features_predications = getDataForSL(csv_path, number_of_teachers, folder_path)
...
if __name__ == "__main__":
    data_by_size_graph("Music+evaluation_August+30,+2021_03.17.csv", 6, "songs", train_ratio=0.5, random_teachers=False)
